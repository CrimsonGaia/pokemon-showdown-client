...
export class MiniEdit {
	...
	element: HTMLElement;
	/**
	 * Takes the plaintext `textContent` of the element, and renders it in syntax-highlighted form. This must not change the resulting
	 * `textContent`, because the data needs to flow two ways to correctly respond to all the possible ways users can input text.
	 * It should, however, add a trailing `\n` if the text doesn't end with one. This is because HTML ignores trailing newlines, so if
	 * it doesn't already exist and the user types a newline at the end of the text, it wouldn't appear.
	 */
	...
	constructor(el: HTMLElement, options: { setContent: MiniEdit['_setContent'], onKeyDown?: (ev: KeyboardEvent) => void }) {
		...
		this.element.addEventListener('input', () => { this.reformat(); });
	...
	private traverseText(node: Node, callback: (node: Text) => boolean): boolean {
		if (node.nodeType === 3) { if (callback(node as Text)) return true; } 
		else { for (let i = 0, len = node.childNodes.length; i < len; ++i) { if (this.traverseText(node.childNodes[i], callback)) return true; } }
		return false;
	}
	...
	reformat(selection?: MiniEditSelection): void { this.setValue(this.getValue(), selection); }
	...
	select(): void { this.setSelection({ start: 0, end: this.getValue().length }); }
}
...
// We can't use the native undo/redo feature because browsers just get so confused by our syntax highlighting, so we have to reimplement it.
// We can intercept ctrl+z and ctrl+y, but we can't intercept the browser's UI so things like Edit -> Undo will simply not work. I am sorry to say that there is no solution and this is just what webdev is like.
