...
function connectToServer() {
	...
	try { socket = new SockJS(url, [], { timeout: 5 * 60 * 1000 }); } 
	catch { socket = new WebSocket(url.replace('http', 'ws') + '/websocket'); }
	if (socket) {
		socket.onopen = () => {
		...
		socket.onmessage = (e: MessageEvent) => { postMessage({ type: 'message', data: e.data }); };
		socket.onclose = () => { postMessage({ type: 'disconnected' }); }; // scheduleReconnect();
