...
export class BattleBGM {
	 ...
	 * object, but two battles with the same music will have the same HTMLAudioElement object.
	 */
	...
	loopend: number;
	/**
	 * When multiple battles with BGM are open, they will be `isPlaying`, but only the first one will be `isActuallyPlaying`. 
	 * In addition, muting volume or setting BGM volume to 0 will set `isActuallyPlaying` to false.
	 */
	isPlaying = false;
	...
	updateTime() {
		...
		if (progress > this.loopend - 1000) { this.sound.currentTime -= (this.loopend - this.loopstart) / 1000; }
		this.timer = setTimeout(() => { this.updateTime(); }, Math.max(this.loopend - progress, 1));
	}
	static update() {
		const current = BattleSound.currentBgm();
		for (const bgm of BattleSound.bgm) {
			if (bgm.isPlaying) {
				if (bgm === current) { bgm.actuallyResume(); } 
				else { bgm.actuallyPause(); }
			}
		}
	}
}
export const BattleSound = new class {
	...
	playEffect(url: string) { this.playSound(url, this.muted ? 0 : this.effectVolume); }
	...
	currentBgm() {
		...
		for (const bgm of this.bgm) { if (bgm.isPlaying) return bgm; }
		return null;
	}
	// setting
	...
	loudnessPercentToAmplitudePercent(loudnessPercent: number) { // 10 dB is perceived as approximately twice as loud
	...
	setEffectVolume(effectVolume: number) { this.effectVolume = this.loudnessPercentToAmplitudePercent(effectVolume); }
};
if (typeof PS === 'object') {
