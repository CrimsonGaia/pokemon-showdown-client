...
ResourceRoom=function(_PSRoom){
    ...
    connect=function connect(){};return ResourceRoom;
}(PSRoom);var
ResourcePanel=function(_PSRoomPanel){
    function ResourcePanel(){var _this2;for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}_this2=_PSRoomPanel.call.apply(_PSRoomPanel,[this].concat(args))||this;_this2.
    ...
    var keys=Object.keys(BattleChatCommands).sort(function(a,b){
        ...
        return!plugin.endsWith('admin');});for(var _i2=0;_i2<
        keys.length;_i2++){var pluginName=keys[_i2];
            var cmdTable=BattleChatCommands[pluginName];
            if(!cmdTable.length)continue;
            if(pluginName.startsWith('chat-plugins')){ pluginName='Chat plugin: '+pluginName.split('/').slice(1).join('/'); }
        else if(pluginName.startsWith('chat-commands')){ pluginName='Core commands: '+pluginName.split('/').slice(1).join('/');; }
        var matchedCmds=[];for(var _i4=0;_i4<
        cmdTable.length;_i4++){
            var cmd=cmdTable[_i4];
            if(search.length&&!toID(cmd).includes(search)){ continue; }
...
}(PSRoomPanel);ResourcePanel.id='resources';ResourcePanel.routes=['resources'];ResourcePanel.Model=ResourceRoom;ResourcePanel.icon=preact.h("i",{"class":"fa fa-question-circle","aria-hidden":true});ResourcePanel.title='Resources';
