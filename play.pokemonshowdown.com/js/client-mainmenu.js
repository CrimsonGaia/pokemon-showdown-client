(function ($) {
	this.MainMenuRoom = this.Room.extend({
		...
		initialize: function () {
			...
			if (app.down) {
				...
				if (app.down === 'ddos') { buf += '<p class="error"><strong>Pok&eacute;mon Showdown is offline due to a DDoS attack!</strong></p>'; } 
				else { buf += '<p class="error"><strong>Pok&eacute;mon Showdown is offline due to technical difficulties!</strong></p>'; }
			...
			// right menu
			if (document.location.hostname === Config.routes.client) { this.$('.rightmenu').html('<div class="menugroup"><p><button class="button mainmenu1 onlineonly disabled" name="joinRoom" value="rooms">Join chat</button></p></div>'); } 
			else { this.$('.rightmenu').html('<div class="menugroup"><p><button class="button mainmenu1 onlineonly disabled" name="joinRoom" value="lobby">Join lobby chat</button></p></div>'); }
			// footer
			// (created during page load)
			...
			Storage.whenPrefsLoaded(function () {
				...
				if (!newsid) {
					if ($(window).width() < 628) { self.minimizePM($news); } // News starts minimized in phone layout
					return;
				}
			...
			if (!app.roomsFirstOpen && window.location.host !== 'demo.psim.us' && window.innerWidth < 630) {
				if (Config.roomsFirstOpenScript) { Config.roomsFirstOpenScript(true); }
			...
			$.ajax({
				...
				success: function (data) {
					...
					for (var i = 0; i < 2; i++) {
						...
						if (post.date) { html += '<small class="date"> on ' + new Date(post.date * 1000).toDateString() + '</small>'; }
						html += '</p></div>';
					}
					self.addPseudoPM({
						title: 'Latest News',
		...
		addPM: function (name, message, target) {
			...
			if (textContent && app.curSideRoom && app.curSideRoom.addPM && Dex.prefs('inchatpm')) { app.curSideRoom.addPM(name, message, target); }
			if (canNotify && !isSelf && textContent) { this.notifyOnce("PM from " + name, "\"" + textContent + "\"", 'pm'); }
			if (autoscroll) { $chatFrame.scrollTop($chat.height()); }
			if (!$pmWindow.hasClass('focused') && name.substr(1) !== app.user.get('name')) { $pmWindow.find('h3').addClass('pm-notifying'); }
		},
		updateChallenge: function ($pmWindow, challenge, name, oName) {
			...
			if ($challenge.find('button[name=makeChallenge]').length) {
				...
				$challenge.find('button[name=dismissChallenge]').attr('data-pendingchallenge', challenge ? (name + '|' + oName + '|' + challenge) : '');
				return;
			}
			...
			if (userid !== oUserid) { // we are sending the challenge
				var buf = '<form class="battleform"><p>Waiting for ' + BattleLog.escapeHTML(oName) + '...</p>';
				if (formatName) { buf += '<p><label class="label">' + (teamFormat ? 'Format' : 'Game') + ':</label>' + this.renderFormats(formatName, true) + '</p>'; }
		...
		openPM: function (name, dontFocus) {
			...
			if (!$pmWindow.length) {
				...
				if (group === ' ') { group = ''; } 
					...
					group = '';
					name = ' ' + name;
				} else { group = '<small>' + BattleLog.escapeHTML(group) + '</small>'; }
		...
		closePM: function (e) {
			...
			if (!userid) {
				...
				if (newsId) { $.cookie('showdown_readnews', '' + newsId, { expires: 365 }); }
				$pmWindow.remove();
				return;
			}
			...
			if ($rejectButton.length) { this.rejectChallenge(userid, $rejectButton); }
			$rejectButton = $pmWindow.find('button[name=cancelChallenge]');
			if ($rejectButton.length) { this.cancelChallenge(userid, $rejectButton); }
			var $next = $pmWindow.next();
			while ($next.length && $next.css('display') === 'none') { $next = $next.next(); }
			if ($next.length) {
				$next.find('textarea[name=message]').focus();
				return;
			}
			$next = $pmWindow.prev();
			while ($next.length && $next.css('display') === 'none') { $next = $next.prev(); }
			if ($next.length) {
				$next.find('textarea[name=message]').focus();
				return;
			}
		...
		focusPM: function (name) { this.openPM(name).prependTo(this.$pmBox).find('textarea[name=message]').focus(); },
		onFocusPM: function (e) { $(e.currentTarget).closest('.pm-window').addClass('focused').find('h3').removeClass('pm-notifying'); },
		onBlurPM: function (e) { $(e.currentTarget).closest('.pm-window').removeClass('focused'); },
		keyUp: function (e) {
		...
		submitPM: function (e) {
			...
			case 'ignore':
				if (app.ignore[userid]) { $chat.append('<div class="chat">User ' + userid + ' is already on your ignore list. (Moderator messages will not be ignored.)</div>'); } 
				else {
					app.ignore[userid] = 1;
				...
				break;
			case 'unignore':
				if (!app.ignore[userid]) { $chat.append('<div class="chat">User ' + userid + ' isn\'t on your ignore list.</div>'); } 
				else {
				...
				break;
			case 'nick':
				if ($.trim(data)) { app.user.rename(data); } 
				else { app.addPopup(LoginPopup); }
				return false;
			case 'chal':
			case 'chall':
			case 'challenge':
		...
		clickPMBackground: function (e) {
			if (!e.shiftKey && !e.cmdKey && !e.ctrlKey) {
				...
				if ($target.data('minimized')) { this.minimizePM(e); } 
					...
					if (Storage.prefs('newsid', newsid)) { $target.find('.unread').removeClass('unread'); }
					return;
				}
		...
		clickSpoiler: function (e) { $(e.currentTarget).toggleClass('spoiler-shown'); },
		clickDatasearchResults: function (e) {
			...
			for (var i = 0; i < results.length; i++) { results[i].style.display = results[i].style.display === 'none' ? 'block' : 'none'; }
		},
		// support for buttons that can be sent by the server:
		joinRoom: function (room) { app.joinRoom(room); },
		avatars: function () { app.addPopup(AvatarsPopup); },
		openSounds: function () { app.addPopup(SoundsPopup, { type: 'semimodal' }); },
		openOptions: function () { app.addPopup(OptionsPopup, { type: 'semimodal' }); },
		...
		resetPending: function () {
			...
			this.$('button[name=acceptChallenge]').each(function (i, el) { el.disabled = false; });
		},
		searching: false,
		...
		updateChallenges: function (data) {
			...
			this.$('.pm-window').each(function (i, el) {
				...
				if (data.challengesFrom[userid]) {
					...
					if (!$pmWindow.find('.challenge').length) { self.notifyOnce("Challenge from " + name, "Format: " + BattleLog.escapeFormat(format), 'challenge:' + userid); }
		...
		openChallenge: function (name, $pmWindow) {
			...
			var $challenge = $pmWindow.find('.challenge');
			if (!$challenge.length) { $challenge = $('<div class="challenge"></div>').insertAfter($pmWindow.find('h3')); }
			return $challenge;
		},
		...
		reconnect: function () { document.location.reload(); },
		...
		updateRightMenu: function () {
			if (app.curSideRoom) { this.$('.rightmenu').hide(); } 
			else { this.$('.rightmenu').show(); }
		},
		// challenge buttons
		challenge: function (name, format, team) {
			var userid = toID(name);
			var $challenge = this.$('.pm-window-' + userid + ' .challenge');
			if ($challenge.length && !$challenge.find('button[name=dismissChallenge]').length) { return; }
			if (format) {
		...
		makeChallenge: function (i, target) {
			...
			app.sendTeam(team, function () { app.send(privacy + '/challenge ' + userid + ', ' + format); });
		},
		...
		format: function (format, button) { if (window.BattleFormats) app.addPopup(FormatPopup, { format: format, sourceEl: button }); },
		...
		renderFormats: function (formatid, noChoice) {
			if (!window.BattleFormats) { return '<button class="select formatselect" name="format" disabled value="' + BattleLog.escapeHTML(formatid) + '"><em>Loading...</em></button>'; }
			if (_.isEmpty(BattleFormats)) { return '<button class="select formatselect" name="format" disabled><em>No formats available</em></button>'; }
			if (!noChoice) {
				this.curFormat = formatid;
				if (!this.curFormat) {
					if (BattleFormats['gen9randombattle']) { this.curFormat = 'gen9randombattle'; } 
		...
		renderTeams: function (formatid, teamIndex) {
			...
			if (!Storage.teams || !window.BattleFormats) { return '<button class="select teamselect" name="team" disabled><em>Loading...</em></button>'; }
			if (!formatid) formatid = this.curFormat;
			var atIndex = formatid.indexOf('@@@');
			if (atIndex >= 0) formatid = formatid.slice(0, atIndex);
			if (!window.BattleFormats[formatid]) { return '<button class="select teamselect" name="team" disabled></button>'; }
			if (window.BattleFormats[formatid].team) { return '<button class="select teamselect preselected" name="team" value="random" disabled>' + TeamPopup.renderTeam('random') + '</button>'; }
			...
			if (!teams.length) { return '<button class="select teamselect" name="team" disabled>You have no teams</button>'; }
			if (teamIndex === undefined) teamIndex = -1;
			if (teamIndex < 0) {
				if (this.curTeamIndex >= 0) { teamIndex = this.curTeamIndex; }
		...
		search: function (i, button) {
			...
			if ($searchForm.find('.cancel').length) { return; }
			...
			if (!window.BattleFormats[format].team && (teamIndex === '' || !team)) {
				if (Storage.teams) { app.addPopupMessage("Please select a team."); } 
				else { app.addPopupMessage("You need to go into the Teambuilder and build a team for this format."); }
				return;
			}
			$formatButton.addClass('preselected')[0].disabled = true;
			$teamButton.addClass('preselected')[0].disabled = true;
			...
			var self = this;
			app.sendTeam(team, function () { self.searchDelay = setTimeout(function () { app.send(self.adjustPrivacy($privacyCheckbox.is(':checked')) + '/search ' + format); }, 3000); });
		},
		...
		parseChatMessage: function (message, name, timestamp, isHighlighted, $chatElem, isNotPM) {
			...
			case 'html':
				if (!name) { return { message: '<div class="chat' + hlClass + '">' + timestamp + '<em>' + BattleLog.sanitizeHTML(target) + '</em></div>', noNotify: isNotPM }; }
			...
			case 'uhtmlchange':
				...
				if (!html) { $elements.remove(); } 
				else if (!$elements.length) { $chatElem.append('<div class="chat uhtml-' + toID(parts[0]) + ' chatmessage-' + toID(name) + '">' + BattleLog.sanitizeHTML(html) + '</div>'); } 
			...
			default:
				...
				if (!name) { return '<div class="chat' + hlClass + '">' + timestamp + '<em>' + BattleLog.parseMessage(message) + '</em></div>'; }
	...
	var FormatPopup = this.FormatPopup = this.Popup.extend({
		...
		renderFormats: function () {
			...
			var bufs = [];
			var curBuf = 0;
			if (this.selectType === 'watch' && !this.search) { bufs[1] = '<li><button name="selectFormat" value="" class="option' + (curFormat === '' ? ' cur' : '') + '">(All formats)</button></li>'; }
			for (var i in this.starred) {
				...
				if (this.search && !i.includes(toID(this.search))) { continue; }
			...
			for (var i in BattleFormats) {
				...
				if (format.section && format.section !== curSection) {
					...
					if (!app.supports['formatColumns']) { curBuf = (curSection === 'Doubles' || curSection === 'Past Generations') ? 2 : 1; } 
					else { curBuf = format.column || 1; }
					if (!bufs[curBuf]) { bufs[curBuf] = ''; }
			...
			var html = '';
			if (bufs.every(function (buf) { return !buf; })) { html = '<ul class="popupmenu"><em>No formats found</em></ul>'; } 
			else {
		...
		updateStar: function (ev) {
			...
			if (this.starred[format]) { delete this.starred[format]; } 
			else { this.starred[format] = true; }
		...
		shouldDisplayFormat: function (format) {
			if (this.selectType === 'teambuilder') { if (!format.isTeambuilderFormat) return false; } 
			else {
		...
		selectFormat: function (format) {
			if (this.onselect) { this.onselect(format); } 
				...
				if ($teraPreviewCheckbox) {
					...
					if (teraPreviewDefault) { $parentTag.removeClass('hidden'); } 
					else {
						$parentTag.addClass('hidden');
				...
				$partnerLabels.each(function (i, label) { label.style.display = BattleFormats[format].partner ? '' : 'none'; });
			}
	...
	var TeamPopup = this.TeamPopup = this.Popup.extend({
		initialize: function (data) {
			...
			var teams = Storage.teams;
			var bufBoundary = 128;
			if (teams.length > 128 && $(window).width() > 1080) { bufBoundary = Math.ceil(teams.length / 5); } 
			else if (teams.length > 81) { bufBoundary = Math.ceil(teams.length / 4); } 
			else if (teams.length > 54) { bufBoundary = Math.ceil(teams.length / 3); } 
			else if (teams.length > 27) { bufBoundary = Math.ceil(teams.length / 2); }
			...
			if (!teams.length) {
				...
				if (teamFormat) {
					...
					if (this.folderToggleOn) {
						...
						if (!isNoFolder) { bufs[curBuf] += '<li><button name="selectFolder" class="button" value="(No Folder)"><i class="fa fa-folder-open" style="margin-right: 7px; margin-left: 4px;"></i>(No Folder)</button></li>'; } 
						else {
				...
				} else { bufs[curBuf] += '<li><button name="moreTeams" class="button">Show all teams</button></li>'; }
			}
			if (format.canUseRandomTeam) { bufs[curBuf] += '<li><button value="-1">Random Team</button></li>'; }
			if (bufs[1]) {
			...
			} else { this.$el.html('<ul class="popupmenu">' + bufs[0] + '</ul>'); }
		},
		events: { 'click input[type=checkbox]': 'foldersToggle' },
		moreTeams: function () {
			this.close();
		...
		teambuilder: function () {
			...
			if (!teambuilder.exportMode && !teambuilder.curTeam && teamFormat) { teambuilder.selectFolder(teamFormat); }
		},
		selectFolder: function (key) {
			...
			if (!keyExists) { folderNotExpanded.push(key); }
			this.close();
		...
		renderTeam: function (i) {
			if (i === 'random') {
				...
				for (var i = 0; i < 6; i++) { buf += '<span class="picon" style="float:left;' + Dex.getPokemonIcon() + '"></span>'; }
				buf += '</small>';
				return buf;
			}
			if (i < 0) { return '<em>Select a team</em>'; }
